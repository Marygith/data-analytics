...
def calculate_dm_deliveryman_income():
    ...
    def process_deliveryman_income():
        ...
        #dm_fact = dds_metadata.tables['dds.dm_fact_table']
        dm_orders = dds_metadata.tables['dds.dm_orders']
        dm_time= dds_metadata.tables['dds.dm_time']
        dm_deliveries = dds_metadata.tables['dds.dm_deliveries']
        dm_deliveryman = dds_metadata.tables['dds.dm_deliverymen']
        deliveryman_income = cdm_metadata.tables['cdm.deliveryman_income']

        # Очистка данных в витрине deliveryman_income_table
        cdm_session.execute(deliveryman_income.delete())
        cdm_session.commit()

        now_time = datetime.now()

        # Get the oldest order date
        oldest_order_date_query = select([dm_time.c.time_mark]).select_from(
            dm_time.join(dm_orders, dm_time.c.id == dm_orders.c.time_id)
        ).where(dm_orders.c.status == 'CLOSE').order_by(dm_time.c.time_mark).limit(1)
        ...
        while current_period_end < now_time:
            ...
            query = select([
                dm_deliveryman.c.id.label('deliveryman_id'),
                dm_deliveryman.c.name.label('deliveryman_name'),
                text('AVG(dm_deliveries.rating) as rating'),
                text('SUM(dm_deliveries.tips) as tips'),
                dm_orders.c.id.label('order_id'),
                dm_time.c.time_mark
            ]).select_from(
                dm_deliveryman.join(dm_deliveries,
                    dm_deliveryman.c.id == dm_deliveries.c.deliveryman_id)
                .join(dm_orders, dm_deliveries.c.id == dm_orders.c.delivery_id)
                .join(dm_time, dm_orders.c.time_id == dm_time.c.id)
            ).where(
                and_(
                    dm_orders.c.status == 'CLOSE',
                    dm_time.c.time_mark.between(current_period_start, current_period_end)
                )
            ).group_by(
                dm_deliveryman.c.id,
                dm_deliveryman.c.name,
                dm_orders.c.id,
                dm_time.c.time_mark
            )
            ...
            for result in results:
                ...
                orders_total_cost_query = select([
                    text('SUM(dm_orders.total_amount) as orders_total_cost')
                ]).select_from(
                    dm_orders
                )
            ...
            for deliveryman_id, data in income_data.items():
                ...
                insert_stmt = deliveryman_income.insert().values(**deliveryman_income_record)
...
cdm_delivery_income_dag = calculate_dm_deliveryman_income()
